################################## REWARDS #####################################
# The classes that handle RD and exploration rewards obtainable by the player.
################################################################################
...
init python:
    ...
    class RewardList:
        ...
        def get_stat_requirement(self, reward_name):
            for reward in self.pending_rewards + self.obtained_rewards:
                if reward_name == reward.name:
                    return reward.stat_requirement, reward.material_requirement
            return Stats(), 0
