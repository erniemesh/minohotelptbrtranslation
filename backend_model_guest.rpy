################################### GUESTS #####################################
# The Guest and GuestList classes that manage guests, their stats, and routes.
################################################################################
...
init python:
    ...
    class GuestList:
        ...
        def names(self):
            ...
            return sort_list_by_key(names_list, preset_guest_names)
        ...
        def increase_guest_stat(self, guest_name, stat, amount):
            self.guests[guest_name].stats.increase_stat(stat, amount)
        ...
        def advance_day(self, days=1):
            for guest_name in self.guests.keys():
                self.guests[guest_name].advance_day(days)
        ...
        def get_guest_routes_not_currently_available(self, current_build_route_max):
            guest_list = []
            for guest_name in self.guests.keys():
                if (self.guests[guest_name].route_progress == current_build_route_max[guest_name]) and not self.guests[guest_name].route_complete:
                    guest_list.append(guest_name)
            return guest_list
